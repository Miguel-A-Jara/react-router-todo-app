...
const ItemTodoTitle = ({todo}) => {
  	return (
		<h1 
			...
			<small className='w-full block text-lg md:text-2xl lg:text-4xl break-words'>{todo.todoTitle}</small>

			{ todo.done 
				? <small className="font-sans text-xs lg:text-lg font-extralight">(Completed)</small> 
				: <small className="font-sans text-xs lg:text-lg font-extralight">(Not Completed)</small> 
			}
		</h1>
  )
}
